...
top_text_metrics = textmetrics(text=top_text, font=font_name, size=font_size);
...
module render() {
    ...
    top_text_y = text_start_y + bottom_text_height + text_spacing;
    translate([top_text_x, top_text_y, layer_height]) {
        color("black")
        linear_extrude(height=layer_height)
        offset(delta=text_thickness)
        text(top_text, font=font_name, size=font_size);
    }
